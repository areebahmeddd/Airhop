...
// `end`) resolve per direction. Those are what the codebase uses now; the
// physical `marginLeft` / `right` forms it used before did not flip, so a
// badge pinned to the trailing corner stayed on the visual right in Arabic and
// covered the wrong thing.
//
