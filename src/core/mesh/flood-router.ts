...
//   - Every incoming packet whose nonce has not been seen is relayed.
...
//   - Packets seen before (same nonce within 5 min) are dropped silently.
//
...
export class FloodRouter {
  ...
  // Scheduled relay timers, keyed by nonce hex. Stored so callers can flush on
  // shutdown if needed.
  ...
  receive(packet: Packet, send: SendFn): boolean {
    if (this.dedup.has(packet.nonce)) return false;
    this.dedup.add(packet.nonce);

    if (packet.ttl > 1) {
      this.scheduleRelay({ ...packet, ttl: packet.ttl - 1 }, send);
    }

    return true;
  }

  // Originate a packet from this node. Records the nonce so we do not relay
  // our own broadcasts back to ourselves.
  originate(packet: Packet): void {
    this.dedup.add(packet.nonce);
  }

  private scheduleRelay(packet: Packet, send: SendFn): void {
    const nonceKey = Array.from(packet.nonce)
    ...
    this.pending.set(nonceKey, timer);
  }
