...
| Mesh DM retry after silent session discard                      | Limitation  | A DM sent over a Noise session the peer silently discarded (e.g. after a restart) is not resent once the session re-establishes. bitchat retries such DMs until an authenticated delivery ack (#1462); Airhop only retries the courier/Nostr tiers, not in-session mesh DMs. Rare in practice.                                                                                                                                                                                                                                                                                                                                                                 | Allows an in-flight mesh DM to be lost across a peer restart with no automatic resend.                                                                |
...
| BLE advertising is not on every Android device                  | Platform    | Some devices at the API 26 floor ship chipsets or firmware with no BLE peripheral support, so `bluetoothLeAdvertiser` is null. These devices scan and receive normally but never advertise.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Leaves affected devices able to join a mesh but not to be discovered by others.                                                                       |
| Peer ID in the advert differs per platform                      | Platform    | Android carries 8 bytes of the peer ID in scan-response service data. CoreBluetooth has no service-data API, so iOS uses the local name instead. Neither platform can read the other's placement.                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Skips advert-level dedup on cross-platform links until the first ANNOUNCE.                                                                            |
...
| ~~`AnnounceManager.buildPacket()` sent no TLV 0x04~~            | Fixed       | `buildPacket()` now supports neighbor IDs and topology gossip via `buildAnnouncePayloadWithNeighbors()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Restores topology gossip, wire-compatible with bitchat's TLV 0x04.                                                                                    |
