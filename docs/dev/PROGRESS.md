...
- [ ] QR display and scan for tokens
...
| Internet gateway (uplink)                             | Limitation | Airhop relays nearby bitchat gateway traffic to Nostr but cannot yet originate gateway carriers for its own offline devices.                                                                                                                                                             | Partial gateway functionality.                                                   |
| Mesh Noise handshake over multi-hop                   | Limitation | New Noise sessions can only be established over a direct BLE link. First-contact DMs to peers reachable only through relays fall back to Nostr/courier instead of creating a mesh session. Existing encrypted sessions continue to relay normally.                                       | First-contact DMs to multi-hop-only peers use the internet path instead of mesh. |
...
| Tor on Android needs Orbot                            | Dependency | iOS embeds Arti, but Android currently relies on Orbot running in VPN mode. Planned to be replaced with an embedded Arti library in v1.2.0.                                                                                                                                              | Android Tor requires a third-party app until v1.2.0; iOS is self-contained.      |
