...
| Tor on Android needs Orbot                                      | Dependency  | iOS embeds Arti directly; Android relies on Orbot running in VPN mode. Enabling is gated on Orbot installed and VPN active, so Tor never turns on into clearnet. A mid-session drop is now detected as it happens: while Tor is on, a `ConnectivityManager.NetworkCallback` on `TRANSPORT_VPN` reports the transport going away and JS re-probes rather than acting on the event alone, since another VPN losing its link is not Orbot stopping. Registered only while the Tor claim is live, and torn down in `invalidate()`. What remains a dependency is Orbot itself: Airhop cannot start it, and cannot prove a live VPN belongs to it rather than to something else running alongside. | Requires a third-party app for Tor on Android; iOS is self-contained.                                                                                                                                                                                             |
...
| ~~WiFi not in `MessageRouter` priority chain~~                  | Fixed       | No change was required; `MessageRouter` already prioritizes WiFi over BLE through `MeshService`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Leaves transport priority unchanged: both radios carry the same packet, then Nostr, then courier.                                                                                                                                                                 |
