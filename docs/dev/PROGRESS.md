...
| No Tor bridges or pluggable transports                          | Gap         | Neither platform can reach Tor through a bridge yet. The Arti build now compiles `bridge-client` and `pt-client` in, so the engine is capable; what is missing is the bridge configuration, the obfs4 or webtunnel transport itself, and a settings surface to enter a bridge line. A direct Tor connection is identifiable by consumer-grade deep packet inspection. bitchat has the same gap on both its platforms, and calls it their largest remaining one.                                                                                                                                                                                                | Leaves Tor use itself visible to network observers, which is why Tor stays off by default on both platforms.                                                                                                                                                      |
...
| ~~Tor depended on Orbot for Android~~                           | Fixed       | Android proxied into Orbot and could only infer whether it was routing, from "some VPN exists" plus "something answers on 9050". Both can be true with Orbot idle beside an unrelated VPN, so the banner could read "Tor on" over clear-net traffic. Airhop now embeds Arti on both platforms from one Rust crate in `native/arti/`, and the proxy is installed into the OkHttp client every Android socket is built from, so `fetch` is covered as well as relay traffic.                                                                                                                                                                                     | Removes the third-party dependency, makes the Tor indicator provable rather than inferred, and covers mint and update traffic on Android that Orbot only covered while it happened to be running.                                                                 |
