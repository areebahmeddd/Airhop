...
| No Tor bridges or pluggable transports                          | Gap         | Neither platform can reach Tor through a bridge. The vendored Arti build ships without pluggable-transport support, and the Android path inherits whatever Orbot is configured to do, which the app neither sets nor can verify. A direct connection to the Tor network is identifiable by consumer-grade deep packet inspection, so where that inspection is deployed, using Tor is itself visible. Fixing it needs an Arti build with pluggable transports plus a bridge configuration surface. bitchat has the same gap and documents it.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Tor hides your IP from the relay; it does not hide Tor from whoever carries your traffic. This is why Tor is off by default on both platforms.                                                                                                                                                   |
| Nearby alerts cannot be silenced per device                     | Gap         | The nearby notification fires on a 0 to 1 peer-count edge and counts peers without distinguishing them. Someone running Airhop on a phone and a tablet gets alerted about their own second device, repeatedly, whenever either radio flaps. The planned fix is a per-peer `notifyOnProximity` flag defaulting on, surfaced as an "Ignore in nearby alerts" row on that peer's info sheet, with ignored peers subtracted from the count before the edge test. Deliberately a notification filter and nothing more: the devices stay separate peers with separate keys, which is the larger multi-device identity question.                                                                                                                                                                                                                                                                                                                                                                         | Only affects users running two or more devices. A true alert about a stranger arriving still fires; the noise trains people to ignore the one notification that matters.                                                                                                                         |
